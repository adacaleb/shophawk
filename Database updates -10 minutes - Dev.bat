...
sqlcmd -S GEARSERVER\SQLEXPRESS -d PRODUCTION -E -Q "SELECT [Document] ,[Customer] ,[Ship_Via] ,[Terms] ,[Document_Date] ,[Due_Date] ,[Orig_Invoice_Amt] ,[Open_Invoice_Amt] ,[Last_Updated] ,[Invoiced_By] FROM [PRODUCTION].[dbo].[Invoice_Header] WHERE Open_Invoice_Amt > 0 ORDER BY Customer ASC " -o "C:\railsapps\shophawkdev\app\assets\csv\Open_Invoice_Amounts.csv" -W -w 1024 -s "`"